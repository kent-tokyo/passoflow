...
Rustのシナリオ契約は、オプトインの`passoflow` package（Pythonからのimport moduleは`passoflow_python`）で利用できます。Rust版の検証をPythonから使う場合は、`rust/crates/passoflow-python`で`maturin develop`を実行してください。
