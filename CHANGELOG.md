...
- Added filesystem-independent Rust expansion for nested scenarios with fail-closed target checks.
- Delegated nested scenario expansion from the opt-in Python bridge to the Rust binding.
- Added a fail-closed path-boundary check while collecting nested scenario sources.
