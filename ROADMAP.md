...
- [x] Freeze the YAML schema, reserved keys, action names, and three-state action outcome contract.
...
- [x] Export the Rust warning contract through the Python package namespace, type stub, example, and binding guide.
...
- [x] Preserve guarded `--start`/`--end` reruns on the opt-in Rust execution path.
...
- [x] Keep the UI smoke contract check aligned with the dependency-free Python action contract module.
...
- [x] Fail closed when a screen-only input adapter receives an active-window coordinate.
- [x] Keep unsupported-platform errors explicit for every coordinate space.
...
- [x] Expose read-only native input capability and permission diagnostics through the Python binding.
