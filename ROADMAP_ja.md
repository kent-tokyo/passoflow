...
- [x] YAMLスキーマ、予約キー、アクション名、3状態のアクション結果契約を固定する。
...
- [x] オプトインRust実行経路でもguard付き`--start`/`--end`再実行を維持する。
...
- [x] 座標系、表示倍率、マルチモニタ、キーボード配列、権限エラーを定義する。未解決の座標系とdenied権限はfail-closedにする。
- [x] 独立したdry-run入力テストツールと共通安全テストを追加する。
- [x] 赤丸表示とfail-safeをPassoFlow独自機能として維持する。表示は常駐オーバーレイのTkウィンドウを再利用し、クリックごとの起動・破棄コストをなくす。
...
- [x] 遷移、クリック、入力、待機、セレクタプレビュー、復旧診断をRust DOM境界へ移植する。
...
- [x] 保護されたGitHub Actions workflowを使い、`passoflow` 0.1.3をcrates.ioへ公開する。
- [x] `CARGO_REGISTRY_TOKEN`を使う`passoflow-core`の公開経路を追加し、Python bindingはPyPI wheel workflowで公開する。
- [x] Rust publish workflowにVERSION/Cargo metadataの事前検証を追加する。
- [x] platform wheelをPyPIへ公開し、クリーンな仮想環境で導入を検証する。
- [x] 0.1.3 wheelをビルドし、クリーンな仮想環境で公開APIを検証する。
...
- [x] Linux、Windows、macOS wheelを個別runnerで検証するrelease workflow smoke testを追加する。
