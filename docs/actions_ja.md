...
`path`または`images`で参照するシナリオファイルと画像テンプレートは、
scenario root配下に置く必要があります。絶対パス、`..`による脱出、
symlink経由のroot外参照は実行前に拒否されます。
...
| `click_indicator_duration` | `0.25` | `click_image`でクリック前に赤丸を表示する秒数。常駐オーバーレイのウィンドウをクリック間で再利用するため、表示を維持しながらクリックごとのTk起動コストを抑える。録画やデバッグ時は増やし、`0`で非表示にできる |
